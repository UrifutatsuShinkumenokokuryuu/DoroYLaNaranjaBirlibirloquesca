...
public class EnemyFollow : MonoBehaviour
{
    ...
    void Update()
    {        
        Vector2 direction = (Vector2)player.transform.position - (Vector2)Enemy.transform.position;           // 1. Calcular dirección hacia el jugador (Vector2)        
        direction = direction.normalized;                                                                    // 2. Normalizar la dirección
        Enemy.transform.position = (Vector2)Enemy.transform.position + direction * speed * Time.deltaTime;  // 3. Mover al enemigo usando solo Vector2        
    }
